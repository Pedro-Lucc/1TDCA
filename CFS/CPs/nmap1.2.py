    ...
    def startthreading(self):
        ...
        for i in range(len(self.port)):
            ...
            if int(threading.activeCount()) == int(self.thread):
                printingline = "\r< IP : {} | Number of Threads : {} | Open Port Checking : {} | Open Ports Founded {} >".format(
                    self.target, str(threading.activeCount()), str(i), str(len(self.store_open_ports)))
...
class BannerGrabber:
    ...
    def start_threading(self, address, port):
        ...
        for i in port:
            ...
            storethread.start()
            if int(threading.activeCount()) == int(self.thread):
                printingline = "\r< IP : {} | Number of Threads : {} | Port : {}>".format(address,
                                                                                          threading.activeCount(), i)
                sys.stdout.write(printingline)
                sys.stdout.flush()
                time.sleep(float(2))
            listthread.append(storethread)
...
def main():
    ...
    parser.add_option("-p", "--port", type='string', dest="port",
                      ...
                      default=None)
    parser.add_option('-n', "--thread", type='string', dest="thread", help="Specify Number of Thread For Scanning ",
                      default='500')
    parser.add_option('-o', "--output", type='string', dest="output", help="Specify Path For Saving Output in Txt.",
                      default=None)
    parser.add_option('-c', "--check", type='string', dest="check",
                      help="Specify False Or 0 If Provided Open Ports Are Already TESTED. Or if open ports are not verified then, don't use this argument.",
                      default=None)
    parser.add_option('-T', '--timeout', type='string', dest="timeout", help="Specify Port Time Out Seconds ",
                      default='2')
    ...
    if not options.input:
        if (not options.target):
            print(
                "[*] Please Specify Target. e.g: -t 192.168.10.1 or -t www.site.org \n[*]\t\t or Provide Input File. e.g: -i file.txt")
            sys.exit(0)

        if not options.port and not options.port:
            print(
                "[*] Please Specify Ports Seperated by commas or Provide Range of Ports. eg. 80-120,121,122,123-1200 \n[*]\t\t or Provide Input File. e.g: -i file.txt")
            sys.exit(0)
